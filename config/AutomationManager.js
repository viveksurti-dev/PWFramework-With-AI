...
const listener = require("../utils/services/Listener");
const queueProcessor = require("../utils/services/QueueProcessor");
...
- If the page is a blog, product listing, contact form, or informational page, generate scenarios relevant to THAT page's functionality only.
- Only generate authentication/login scenarios if the DOM clearly shows username and password input fields.
${existingPromptContext}${dataContext}
...
Write the scenarios in this exact format:
{
  ...
  "remarks": "NA"
    ...
    try {
      const cleanJsonStr = fileService.repairJsonArray(responseText);
...
1. "verificationCode": A class named ${verificationClassName} that EXTENDS CommonVerifications.
   - Import: \`import { CommonVerifications } from '../verification/CommonVerifications.js';\`
   - Class declaration: \`export class ${verificationClassName} extends CommonVerifications\`
   - Constructor MUST call \`super(page);\` first.
   - Use inherited helpers (verifyURL, verifyURLPattern, verifyElementVisible, verifyErrorVisible, verifyPageContains) where applicable.
   - Add only scenario-specific verification methods beyond what the base class provides.
   - Export using ES Module named export: \`export class ${verificationClassName} extends CommonVerifications { ... }\`
2. "pageCode": An ES Module exporting a class named ${pageClassName} that EXTENDS CommonMethods.
   - Import: \`import { CommonMethods } from '../pages/CommonMethods.js';\`
   - Class declaration: \`export class ${pageClassName} extends CommonMethods\`
   - Constructor MUST call \`super(page);\` first.
   - Use inherited helpers (fillInput, clickElement, navigateToUrl, isVisible, waitForUrl) instead of raw page calls where possible.
...
`;
    ...
    try {
      ...
      // Use shared JSON repair utility
      responseText = fileService.repairJsonArray(responseText);
...
async function generateSingleExecutableScript(targetUrl, scenariosToAutomate, selectedFile) {
  ...
  const verificationFileName = `${shortName}_Manual_Verif.js`;
  const pageFileName = `${shortName}_ManualPage.js`;
...
1. "verificationCode": A class named ${verificationClassName} that EXTENDS CommonVerifications.
   ...
   - Use inherited helpers (verifyURL, verifyURLPattern, verifyElementVisible, verifyErrorVisible, verifyPageContains) where applicable.
   - Add only scenario-specific verification methods beyond what the base class provides.
   - Export using ES Module named export.
2. "pageCode": An ES Module exporting a class named ${pageClassName} that EXTENDS CommonMethods.
   - Import: \`import { CommonMethods } from '../pages/CommonMethods.js';\`
   - Class declaration: \`export class ${pageClassName} extends CommonMethods\`
   - Constructor MUST call \`super(page);\` first.
...
async function startInteractiveRecorder(url, safeName) {
  return new Promise(async (resolve) => {
    try {
      ...
      // 2. Inject Listener
      await listener.injectSpy(page, safeName);
      ...
      page.on('framenavigated', async (frame) => {
          if (frame === page.mainFrame()) {
              ...
              const state = await listener.handleNavigation(page, safeName);
      ...
      const autoWatcher = setInterval(async () => {
          const idleTime = Date.now() - listener.lastActivityTime;
          if (idleTime > 60000) {
              console.log(`\n-> [Timeout] 60s inactivity detected. Finalizing session...`);
              clearInterval(autoWatcher);
              isFlowGenerated = true; // Set BEFORE generating so page.on('close') skips it
      ...
      page.on('close', async () => {
          ...
          if (!isFlowGenerated) {
              ...
              console.log("-> [Recorder] Flow already generated. Skipping duplicate generation.");
              await flowGenerator.waitForCompletion();
          }

          resolve(); // Return control to the menu loop — no process.exit()
      });
