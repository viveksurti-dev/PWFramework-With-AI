...
test('TC-AUTO-1: Happy Path Navigation to Courses', async ({ page }) => {
  const p = new PracticetestautomationcomPage(page);
  const v = new PracticetestautomationcomVerif(page);
  await p.navigateToTable();
  await v.verifyTablePage();
  await p.clickCourses();
  await v.verifyCoursesPage();
  await p.fillSubscription('John Doe', 'john@example.com');
  await p.submitForm();
  await v.verifyPageContains('Check your inbox');
});